...
Bug fixes
~~~~~~~~~
The upgrade to Bouncy Castle 1.69 in version 9.0.3 turned out to be broken due to some code movement
and introduction of a new ``bcutil-jdk15on`` artifact. It has also highlighted duplicate packaging, where
Karaf's features would install version 1.66, while we have been on a newer version for some time. Both
these issues have now ween resolved. See `ODLPARENT-253 <https://jira.opendaylight.org/browse/ODLPARENT-254>`__
