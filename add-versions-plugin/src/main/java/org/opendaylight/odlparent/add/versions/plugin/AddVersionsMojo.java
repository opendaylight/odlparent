...
public class AddVersionsMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException {
        ...
        final Source srcXslt = new StreamSource(getClass().getClassLoader().getResourceAsStream("feature.xsl"));

        final File dstDir = new File(projectTargetDir, "generated-resources/feature");
        if (!dstDir.mkdirs()) {
            throw new MojoExecutionException("Failed to create directory " + dstDir);
        }
        final Result resultFeature = new StreamResult(new File(dstDir, "feature.xml"));
