...
public class GenerateDescriptorMojo extends AbstractLogEnabled implements Mojo {
    ...
    private void writeFeatures(PrintStream out) throws ArtifactResolutionException, ArtifactNotFoundException,
        ...
        Map<String, Feature> otherFeatures = new HashMap<String, Feature>();
    ...
    private void processFeatureArtifact(Features features, Feature feature, Map<String, Feature> otherFeatures,
        ...
        if (this.dependencyHelper.isArtifactAFeature(artifact) && FEATURE_CLASSIFIER.equals(
            ...
            for (Feature includedFeature : includedFeatures.getFeature()) {
                ...
                dependency.setName(includedFeature.getName());
                dependency.setVersion(includedFeature.getVersion());
                // We musn't de-duplicate here, we may have seen a feature in !add mode
                otherFeatures.put(dependencyToKey(dependency), includedFeature);
                if (add) {
    ...
    private boolean isBundleIncludedTransitively(Feature feature, Map<String, Feature> otherFeatures,
                                                 Bundle bundle) {
        for (Dependency dependency : feature.getFeature()) {
            Feature otherFeature = otherFeatures.get(dependencyToKey(dependency));
    ...
    private String dependencyToKey(Dependency dependency) {
