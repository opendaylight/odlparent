...
public final class ShutdownRegistry implements AutoCloseable {
    ...
    private record Open(ArrayDeque<AutoCloseable> resources) implements State {
        Open {
    ...
    public void close() {
        ...
        synchronized (this) {
            resources = switch (state) {
                ...
                case Open open -> {
                    LOG.debug("Closing instance {}", uuid);
                    state = Closed.INSTANCE;
                    yield open.resources.descendingIterator();
                }
            };
        }
    ...
    public synchronized String toString() {
        return "ShutdownRegistry [uuid=" + uuid + ", state=" + state.getClass().getName() + "]";
    }
}
