...
public final class TestFeaturesMojo extends AbstractMojo {
    ...
    private static final String FEATURE_DIR = "feature";
    ...
    public void execute() throws MojoExecutionException {
        ...
        // Workaround to prevent MalformedUrlException caused by 'mvn' protocol handler is not retrieved
        // via system property mechanism -- See URL#getURLStreamHandler(String)
        ...
        final var projectFeatureFile = getFeatureFile(buildDir);

        final var resolver = new DependencyResolver(repoSystem, repoSession, repositories);
        final var projectFeatureNames = resolver.resolveFeatureFile(projectFeatureFile);
        ...
        LOG.info("Project features detected: {}", projectFeatureNames);
        LOG.info("Project dependency features detected: {}", projectDependencyFeatures);
        LOG.info("Plugin required features resolved: {}", pluginDependencyFeatures);
        ...
        final var container = new KarafTestContainerFactory().create(system);

        final var execution = new PaxExamExecution(buildDir + "/feature", localRepository, List.of(container));
        if (concurrent) {
            execution.execute();
            return;
        }
    ...
    private static File getFeatureFile(String buildDir) throws MojoExecutionException {
        for (var filename : FEATURE_FILENAMES) {
            final File file = new File(buildDir + File.separator + FEATURE_DIR + File.separator + filename);
            if (file.exists()) {
                return file;
            }
        }
        throw new MojoExecutionException("No feature XML file found in " + buildDir + File.separator + FEATURE_DIR);
    }
