...
public final class TestProbe {
    ...
    final class BundleCheck implements BundleListener, AutoCloseable {
            ...
            bundleCheckResults = Arrays.stream(bundleContext.getBundles())
                .collect(Collectors.toMap(Bundle::getBundleId, bundle -> {
                    ...
                    LOG.info("Initial bundle state: [{}]{} -> {}/{} ({})", bundle.getBundleId(), info.getSymbolicName(),
                        bundleStateText(bundle), info.getState(), result);
                    return result;
                }));
        ...
        private void updateBundle(final Bundle bundle) {
            ...
            bundleCheckResults.compute(bundle.getBundleId(), (bundleId, prev) -> {
                if (prev != next) {
                    LOG.info("Bundle state updated: [{}]{} -> {}/{} ({} -> {})", bundleId, info.getSymbolicName(),
                        bundleStateText(bundle), info.getState(), prev, next);
                }
                return next;
            });
        }
        ...
        private synchronized void logNokBundleDetails() {
            ...
            for (var bundle : bundleContext.getBundles()) {
                ...
                if (nokBundles.contains(bundleId)) {
                    final var info = bundleService.getInfo(bundle);
                    final var diag = bundleService.getDiag(bundle);
                    final var dueTo = diag.isEmpty() ? "" : ", due to: " + diag;
                    LOG.warn("NOK Bundle [{}]{} -> {}/{}{}", bundleId, info.getSymbolicName(),
                        bundleStateText(bundle), info.getState(), dueTo);
                }
            }
    ...
    private static String bundleStateText(final Bundle bundle) {
        final var state = bundle.getState();
        return switch (state) {
