...
public class SingleFeatureTest {
    ...
    public TestProbeBuilder probeConfiguration(TestProbeBuilder probe) {
        ...
        // or http://iocanel.blogspot.ch/2012/01/advanced-integration-testing-with-pax.html
        ...
        addAllClassesInSameAndSubPackageOfClass(probe, TestBundleDiag.class);
        addAllClassesInSameAndSubPackageOfClass(probe, Awaitility.class);
        addAllClassesInSameAndSubPackageOfPackage(probe, "com.google.common");

        return probe;
    }

    // TODO factor out the following 4 methods into an e.g. ReflectionUtil; rename test
    ...
    private void addAllClassesInSameAndSubPackageOfClass(TestProbeBuilder probe, Class<?> clazz) {
        addAllClassesInSameAndSubPackageOfPackage(probe, clazz.getPackage().getName());
    }

    private void addAllClassesInSameAndSubPackageOfPackage(TestProbeBuilder probe, String packageName) {
        getClasses(packageName).forEach(eachClass -> probe.addTest(eachClass));
    }
    ...
    private Stream<Class<?>> getDeclaredAndAnonymousInnerClass(Class<?> clazz) {
