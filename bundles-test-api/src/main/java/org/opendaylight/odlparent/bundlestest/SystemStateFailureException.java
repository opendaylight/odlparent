 ...
 * if there are any OSGi bundles that failed to start. This is based on not only
 * a Bundle's active state, but also DI wiring systems such as blueprint
 * containers. The Exceptions' message will likely contain a longer multi-line
 * String with extensive technical details including all failed bundles' states,
 * detailed technical information related to OSGi bundle and blueprint
 * resolution, and possibly exceptions incl. stack traces thrown by bundle
 * activators and dependency injection object wiring {@literal @}PostConstruct
 * "init" type methods.
 *
